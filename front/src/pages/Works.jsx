...
const PostCard = ({ post, onClick, t }) => (
  ...
  >
    <div className="relative w-full h-52 bg-zinc-200 shrink-0 overflow-hidden">
      <img
    ...
    <div className="p-6 flex flex-col flex-grow">
      <h2 className="text-xl font-bold mb-3 text-zinc-900 line-clamp-2 h-[3.5rem] leading-tight group-hover:text-amber-600 transition-colors">
        {post.title}
      </h2>
    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16 bg-zinc-50/50 min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
        ...
        <div className="bg-white px-6 py-3 rounded-2xl border border-zinc-200 shadow-sm self-start">
          <span className="text-zinc-400 text-sm font-medium italic">{t('works.showing')} </span>
          <span className="text-zinc-900 font-black">{filteredPosts.length} {t('works.results')}</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] shadow-xl shadow-zinc-200/50 border border-zinc-100 p-2 mb-12">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
          ...
          <div className="md:col-span-3 border-l border-zinc-100 relative group">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-[450px] bg-white rounded-3xl border border-zinc-100 animate-pulse p-6">
              <div className="w-full h-48 bg-zinc-200 rounded-2xl mb-6" />
              <div className="w-3/4 h-6 bg-zinc-200 rounded-full mb-4" />
              <div className="w-full h-20 bg-zinc-100 rounded-2xl" />
            </div>
          ))}
        </div>
              ...
              <div className="flex gap-2">
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    className={`w-12 h-12 rounded-2xl font-black transition-all ${currentPage === i + 1 ? 'bg-amber-400 text-black shadow-lg shadow-amber-400/20' : 'bg-white text-zinc-400 hover:border-zinc-300 border border-zinc-100'}`}
                  >
                    {i + 1}
                  </button>
                ))}
      ...
      {/* Modal Lateral (Slide-over) Premium */}
      <div
          ...
          {selectedPost && (
            <>
              <div className="relative h-[350px] shrink-0">
                ...
                <button onClick={() => setSelectedPost(null)} className="absolute top-6 right-6 bg-white/10 hover:bg-white/30 backdrop-blur-xl text-white w-12 h-12 rounded-full flex items-center justify-center transition-all">
                  <X size={24} />
                </button>
                <div className="absolute bottom-8 left-8 right-8">
                  <span className="inline-block bg-amber-400 text-black text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter mb-3">
                    {selectedPost.category}
                  </span>
                  <h2 className="text-3xl font-black text-white leading-tight uppercase tracking-tighter">{selectedPost.title}</h2>
                </div>
              </div>

              <div className="p-10 overflow-y-auto flex-grow bg-white rounded-t-[3rem] -mt-10 relative z-10">
                <div className="flex items-center gap-4 mb-10 pb-10 border-b border-zinc-100">
                  <div className="w-14 h-14 bg-zinc-950 rounded-2xl flex items-center justify-center font-black text-xl text-amber-400">
